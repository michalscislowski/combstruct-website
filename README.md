...
Contact links open `kontakt.html`, which uses the existing published `daniel@combstruct.com` mail address. No mail delivery integration is configured, and there is no simulated form submission. The previous source contained a placeholder phone number and a form that only logged data and displayed an alert; these are not exposed in the new deployment.
