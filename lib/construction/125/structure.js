...
export function buildStructure(materials={},options={}){
 ...
 const x0=-6.25,x1=6.25,z0=-5,z1=5,ceilingTop=9*P,ceilingBottom=ceilingTop-H;
 ...
 const specs=normalizeWalls([...exterior,...layout.partitions],{x0,x1,z0,z1});
 ...
 for(const w of specs){
  ...
  wallPanels.push(wallPanel(engine,w,specs,{target,base:0,modules:9,cap:!!w.exterior,holes}));
 }
 ...
 const dimensions={width:12.5,depth:10,module:P,footprint:125,floorTop:H,wallTop:ceilingTop+T,ceilingBottom,ceilingTop,clearHeight:ceilingBottom-H,terraceArea:18.75,terraceWidth:7.5,terraceDepth:2.5,boardDepth:H};
 const validation={module:P,stockMaxLength:2.5,pairWidth:2*T,terminalDifference:H,junctionsCoplanar:true,floorTerraceContinuous:true,shortBearingDirection:'Z',shortBearingSlots:'up',exteriorVerticalSlots:'outward',connectorBoards:boards.filter(b=>b.connectorCuts.length).length,loadValidated:false};
