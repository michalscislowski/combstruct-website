...
const root=document.getElementById('comb-125-model');
...
renderer.domElement.setAttribute('aria-label','Combstruct 125. Konstrukcja z desek Combstruct; przeciągnij, aby obrócić.');stage.append(renderer.domElement);
const reduced=matchMedia('(prefers-reduced-motion: reduce)');
const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=!reduced.matches;controls.dampingFactor=.09;controls.rotateSpeed=.65;controls.minPolarAngle=.03;controls.maxPolarAngle=Math.PI*.53;
...
const surfaces=[...new Set(Object.values(groups).flatMap(groupList))].filter(g=>g!==groups.insulation);
const originals=[];
...
function setView(next){
 if(!['structure','interior','floor','roof','insulation'].includes(next))return false;
 view=next;clipRange=null;renderer.clippingPlanes=[];
 for(const group of surfaces)group.visible=true;
 groups.insulation.visible=next==='insulation';
 if(next==='interior'){groups.ceiling.visible=false;groups.walls[1].visible=false;groups.walls[2].visible=false}
 if(next==='floor'||next==='roof'){for(const group of surfaces)group.visible=false;(next==='floor'?groups.floor:groups.ceiling).visible=true}
 $('[data-view]').value=next;reset();invalidate();return true;
}
...
window.combstruct125={model,setView,setMode:setView,reset,getState:()=>({ready:true,view,boardCount:boards.length,insulationCount:model.insulationCount,visibleInsulation:view==='insulation'?model.insulationCount:0,dimensions,rooms:model.rooms,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,validation:model.validation||null})};
...
loading.hidden=true;resize();setView('structure');render();document.documentElement.dataset.ready='true';
