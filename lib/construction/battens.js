// Wall-face battens, in metres. These are presentation geometry, not a fastener schedule.
...
export function wallBattens(ribs, holes=[]) {
 ...
 for(const rib of ribs){
  const vertical=rib.axis===1,side=vertical?'exterior':'interior';
  ...
  for(const [lo,hi] of joined)for(let start=lo;start<hi-EPS;start+=BATTEN.stockLength){
   ...
   for(const piece of pieces)result.push({...piece,side,axis:rib.axis,rib:rib.index,stockStart:start,stockEnd:end});
  }
 }
 return result;
}
