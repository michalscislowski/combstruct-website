...
import {stockBounds,stockSlots} from './stock-profile.js';
...
// Shared native profiles and staggered stock. Full end slots are the current
// junction strategy; lateral cuts are retained for regression comparisons.
export function createConstruction(materials={},connectorSchedule={},options={}){
 ...
 const boards=[];let nextId=0;
  ...
  function profile(p) {
    ...
    const points = [new THREE.Vector2(-(p.extendStart||0), 0), new THREE.Vector2(length+(p.extendEnd||0), 0), new THREE.Vector2(length+(p.extendEnd||0), HALF), new THREE.Vector2(length - T, HALF), new THREE.Vector2(length - T, H)];
    ...
    points.push(new THREE.Vector2(T, H), new THREE.Vector2(T, HALF), new THREE.Vector2(-(p.extendStart||0), HALF));
    let result = points;
    const [lo,hi]=stockBounds(p);
    if (hi <= lo + EPS) return null;
    if (lo > -(p.extendStart||0)+EPS) result = cutPolygon(result, 'x', lo, 1);
    if (hi < length+(p.extendEnd||0)-EPS) result = cutPolygon(result, 'x', hi, -1);
  ...
  function clippedProfile(p, basis, position, planes, regions = []) {
    const length=p.pixels*P,[low,high]=stockBounds(p);
    if (high <= low + EPS) return null;
    const slots=stockSlots(p);
    ...
    function block(a, b, bottom, top) {
      ...
      // Disjoint convex regions retain the lower lap and relieve only upper
      // corners beside roof slots. Each cell is closed after every plane cut.
      const cutBeforeRegions=actuallyCut;
      const regionCuts=regions.length&&polys.some(face=>face.some(v=>!regions.some(region=>region.every(q=>q.n.dot(v.clone().add(position))<=q.c+EPS))));
      const retained=regions.length?regions.flatMap(region=>{
        let part=polys;
        for(const plane of region){part=clipFaces(part,plane);if(!part.length)break;}
        return part;
      }):polys;
      actuallyCut=cutBeforeRegions||!!regionCuts;
      for (const poly of retained) {
  ...
  function addBoard(target, p, along, normal, thick, origin, meta = {}, phase = 0, planes = [], regions = []) {
    const [stockStart,stockEnd]=stockBounds(p,false);
    if(stockEnd-stockStart>2.5+EPS)throw new Error('Combstruct stock exceeds 2500 mm');
    ...
    const connectorCuts=options.connectors===false||lengthJoints?[]:(connectorSchedule[connectorKey]||[]);
    p={...p,connectorCuts};
    ...
    const clipped = planes.length || regions.length || (p.minCut || 0) > EPS || p.limit !== undefined || connectorCuts.length;
    const geometry = clipped ? clippedProfile(p, basis, position, planes, regions) : profile(p);
    if (!geometry) return null;
    if (!clipped) geometry.applyMatrix4(basis);
    const [stockLow,stockHigh]=stockBounds(p,false),[actualLow,actualHigh]=stockBounds(p);
    const custom=!!geometry.userData.angledCut||actualLow>stockLow+EPS||actualHigh<stockHigh-EPS;
    ...
    const record = {id: `board-${++nextId}`, mesh, surface: target.name, kind: meta.kind || 'standard', axis: meta.axis, index: meta.index, layer, stockLayer: p.layer, start: p.start, stockLength: p.pixels * P, trim: p.trim || null, custom, connectorKey, connectorCuts, stock: {...p}, along: along.toArray(), normal: normal.toArray(), thick: thick.toArray(), frameOrigin: origin.toArray(), clippingPlanes: planes.map(q=>({n:q.n.toArray(),c:q.c})), actualStart:p.start+actualLow,actualEnd:p.start+actualHigh,...meta};
    if(regions.length)record.clippingRegions=regions.map(region=>region.map(q=>({n:q.n.toArray(),c:q.c})));
  ...
  const jointStock = (modules,phase=0,gaps=[],offset=2,terminals)=>planStock(modules,phase,lengthJoints?gaps.map(g=>({...g,fullEndSlot:g.side!=='seam'})):gaps,offset,terminals);

 return{lengthJoints,boards,segments,jointStock,openStock,addBoard,grid};
}
