    ...
    for(const physical of [shortPhysical,other]) {
      ...
      for(let k=0;k<nodes.length-1;k++) {
        ...
        for(let pos=a;pos<b;) {
          ...
          let right=(layer===1&&end===modules)||gaps.some(g=>g.layer===physical&&g.at===end&&g.side==='right');
          if(left&&right){
            const choices=[count-2,count-1,count-3,count-4,count-5].filter(n=>n>0&&!forbidden.includes(pos+n));
            if(!choices.length)throw Error('No native span between junction bands: '+JSON.stringify({modules,phase,gaps,pos,count,physical}));
            count=choices[0];end=pos+count;right=false;
          }
