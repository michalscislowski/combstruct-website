...
export function boardObstacles(boards,transform=null) {
 return boards.map(({id,mesh})=>{
  mesh.updateWorldMatrix(true,false);
  const matrix=transform?transform.clone().multiply(mesh.matrixWorld):mesh.matrixWorld.clone();
  const bounds=new THREE.Box3();
  const position=mesh.geometry.attributes.position,v=new THREE.Vector3();
  for(let i=0;i<position.count;i++)bounds.expandByPoint(v.fromBufferAttribute(position,i).applyMatrix4(matrix));
  return {id,mesh,matrix,bounds,triangles:null};
 });
}

function worldTriangles(solid) {
 if(!solid.triangles) {
  const {geometry}=solid.mesh,matrixWorld=solid.matrix||solid.mesh.matrixWorld,position=geometry.attributes.position,index=geometry.index;
  solid.triangles=[];
...
export function clearBatten(part,solids) {
 const run=part.runAxis??(part.axis===1?'y':part.wallAxis===0?'x':'z'),k=axes.indexOf(run);
