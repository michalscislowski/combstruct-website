...
import {roofRidgeLap,intersectRegions} from '../roof-ridge-lap.js';
...
export function buildStructure(materials={},options={}){
  ...
  const groundCapModule=lengthJoints?7:8;
  ...
  const {envelope:E}=layout,{width:W,depth:D,x0,x1,z0,z1}=E;
  const L=lengthJoints?{...layout.levels,bearing:groundCapModule*P,slabBottom:groundCapModule*P+T,upper:groundCapModule*P+T+H,roofEave:layout.levels.roofEave+H-P,roofRidgeUnderside:layout.levels.roofRidgeUnderside+H-P}:layout.levels;
  const slabShift=0,upperPartitionBase=lengthJoints?groundCapModule*P:L.slabBottom,ceilingBottom=upperPartitionBase+8*P+(lengthJoints?T:-H),ceilingTop=ceilingBottom+H;
  ...
  const roofLap=lengthJoints?roofWallLap({pitch:45,spanHalf:D/2,ridgeUnderside:L.roofRidgeUnderside,axis:2}):null;
  if(roofLap)dimensions.roofWallJoint={strategy:'between-slots',clearance:roofLap.clearance,phaseOrigin:roofLap.phaseOrigin,wallCenter:roofLap.wallCenter,slotInner:roofLap.slotInner,slotOuter:roofLap.slotOuter,slabBeveled:false,knee:true};
  ...
  const specs=normalizeWalls([...layout.exteriorWalls.map(w=>({...w,exterior:true})),...layout.partitions,...supports],{...E,lengthJoints});
  for(const w of specs){
    // Balcony wing walls have free horizontal ends: no crossing upright
    // occupies either end band. Keep two native two-module ordinary boards.
    if(w.support)w.horizontalEnds={left:false,right:false};
    // At the recessed hall corner the crossing wall lies BEFORE each run's
    // start node. The positive run can start full; its far T end still reserves
    // the native half-lock. This extends material without moving the wall axes.
    if(w.id==='upper-hall-notch-west'||w.id==='upper-hall-notch-north')w.horizontalEnds={left:false,right:true};
    // Exchange the ordinary/ending uprights between the two physical plies.
    // These upper walls are one module away from lower-storey walls. Opposite
    // full-end plies let the slab bridge each node without a 176.7 mm fragment.
    // horizontalPanel uses the same phase at the slab AND attic ceiling.
    if(w.id==='upper-hall-notch-west'||w.id==='upper-hall-notch-north')w.verticalPhase=0;
  }
  // End jambs use the same full/short native half-lock in both horizontal
  // plates as every other upright. Reserve their ply before building a slab.
  for(const w of specs)w.endJambs=w.holes.flatMap(h=>{
    const a=w.a[w.axis]+h.at-T,b=a+h.width+2*T;
    return [a,b].filter(p=>Math.abs(p-w.start)<EPS||Math.abs(p-w.end)<EPS);
  });
  ...
  for(const w of specs){
    const isUpper=w.level==='upper',base=isUpper?upperPartitionBase:0;
    const n=w.exterior&&isUpper?Math.ceil((L.roofRidgeUnderside-base)/P)+1:lengthJoints?(isUpper?9:groundCapModule+1):8;
    let target;
    ...
    const holes=w.holes.map(original=>{
      const h={...original};
      if(lengthJoints&&isUpper){
        const {sill,lintel}=h.moduleBounds;
        h.bottom=base+(sill===null?H+T:sill*P+T);
        h.height=base+lintel*P-T-h.bottom;
      }
    ...
    const verticalTreatment=roofLap&&isUpper&&w.exterior&&w.axis===0?layer=>roofLap.side(w.side==='left'?-1:1).wall(layer):undefined;
    const panel=wallPanel(engine,w,specs,{target,base,modules:n,capAt:isUpper&&w.exterior?n:isUpper?8:groundCapModule,cap:lengthJoints||!!(isUpper&&w.exterior),holes,frameOpenings:true,planes:lengthJoints?[...(isUpper?roofPlanes:[]),...(!(isUpper&&w.exterior)?[{n:Y,c:isUpper?ceilingTop:L.upper}]:[])]:isUpper?roofPlanes:[],verticalEnds:lengthJoints?{left:true,right:false,extraTrim:isUpper}:undefined,verticalGaps:lengthJoints?[{at:isUpper?8:groundCapModule,side:'left',layer:1^(w.verticalPhase??1)}]:isUpper&&w.exterior?[{at:8,side:'right',layer:0}]:[],continueFromBelow:continues,verticalTreatment});
    wallPanels.push(panel);
  }
  const angle=Math.PI/4,sine=Math.sin(angle),cosine=Math.cos(angle),roofX=x0-2*P,roofN=27,roofU=roofLap?.phaseOrigin??-P/2,roofRun=D/2+P;
  const slopeModules=Math.ceil((roofRun/cosine-roofU)/P)+1,roofCenterY=L.roofRidgeUnderside+HALF/cosine;
  const ridge=lengthJoints?roofRidgeLap({pitch:45,centerY:roofCenterY,phaseOrigin:roofU,axis:2}):null;
  for(const side of [-1,1]){
    ...
    const roofHoles=side===-1?[7,13,17].map((i,k)=>({id:`rooflight-${k+1}`,kind:'rooflight',u0:6*P+T,u1:8*P-T,v0:(i-1)*P+T,v1:(i+1)*P-T})):[];
    ...
    for(let j=1;j<roofN;j++){
      ...
      for(const stock of segments(ridge?Math.ceil((roofRun/cosine+HALF*sine/cosine+T-ridge.rafterStart)/P):slopeModules)){
        const p={...stock,start:stock.start-(ridge?.headModules||0)*P,trim:null};
        const phase=side===-1?0:1,supported=roofLap&&x>x0+EPS&&x<x1-EPS;
        const treatments=supported?roofLap.side(side).roof(p.layer^phase):[{planes:[],regions:[]}];
        const ridgeEnd=ridge?.treatment(side,p.layer^phase);
        for(const f of openStock(p,0,j*P+((p.layer^phase)===0?-T:0),roofHoles))for(const treatment of treatments)addBoard(target,f,u,n,X,origin.clone().addScaledVector(X,j*P),{kind:'roof-slope',axis:0,index:j,roofSide:side,...(supported?{roofLap:treatment.role}:{}),...(ridgeEnd?{ridgeLap:ridgeEnd.role}:{})},phase,[...(ridgeEnd?[planes[1],...ridgeEnd.planes]:planes),...treatment.planes],intersectRegions(treatment.regions,ridgeEnd?.regions));
      }
    }
  ...
  if(ridge)dimensions.ridgeJoint={strategy:'alternating-ply-lap',pitch:ridge.pitch,headModules:ridge.headModules,nearSlots:ridge.nearSlots,roofEnvelopePreserved:true};
  ...
  const stair=lengthJoints?{...layout.stair,riser:(L.upper-H)/16,upperY:L.upper}:layout.stair,r=stair.riser,walkMat=materials.stair||wood;
