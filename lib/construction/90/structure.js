  ...
  for(const side of [-1,1]){
    ...
    for(let j=1;j<roofN;j++){
      ...
      for(const stock of segments(slopeModules)){
        ...
        for(const f of openStock(p,0,j*P+((p.layer^phase)===0?-T:0),roofHoles))for(const treatment of treatments)addBoard(target,f,u,n,X,origin.clone().addScaledVector(X,j*P),{kind:'roof-slope',axis:0,index:j,roofSide:side,...(supported?{roofLap:treatment.role}:{})},phase,[...planes,...treatment.planes],treatment.regions);
      }
    }
