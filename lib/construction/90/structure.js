...
export function buildStructure(materials={},options={}){
  ...
  for(const w of specs){
    ...
    const panel=wallPanel(engine,w,specs,{target,base,modules:n,capAt:isUpper&&w.exterior?n:isUpper?8:groundCapModule,cap:lengthJoints||!!(isUpper&&w.exterior),holes,frameOpenings:true,planes:lengthJoints?[...(isUpper?roofPlanes:[]),...(!(isUpper&&w.exterior)?[{n:Y,c:isUpper?ceilingTop:L.upper}]:[])]:isUpper?roofPlanes:[],verticalEnds:lengthJoints?{left:true,right:false,extraTrim:isUpper}:undefined,verticalGaps:lengthJoints?[{at:isUpper?8:groundCapModule,side:'left',layer:1^(w.verticalPhase??1)}]:isUpper&&w.exterior?[{at:8,side:'right',layer:0}]:[],continueFromBelow:continues});
