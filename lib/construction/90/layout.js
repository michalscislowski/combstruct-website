...
export const partitions = [
  wall('ground-office-east','ground',[A.leftX,I.rear],[A.leftX,A.officeZ]),
  wall('ground-office-south','ground',[I.left,A.officeZ],[A.leftX-H/2,A.officeZ],[{...door('office-door',A.smallX+H/2-I.left,.85),swing:-1}]),
...
// Rough openings stop at the faces of complete, paired Combstruct ribs.
// These same bounds drive the plan, model and insulation; cutting at arbitrary
// architectural offsets would leave unsupported rib tips instead of jambs.
const openingNodes={
  'kitchen-garden-door':[10,12], 'living-rear-glazing':[15,21],
  entrance:[5,8], 'utility-window':[1,3], 'office-window':[1,5],
  'ground-bath-window':[8,10], 'living-terrace-glazing':[1,8],
  'rear-bedroom-window':[2,6], 'front-bedroom-window':[10,12],
  'master-balcony-door':[7,12], 'office-door':[5,8],
  'bath-door':[8,10], 'utility-door':[12,14], 'vestibule-hall-door':[6,8],
  'rear-bedroom-door':[6,8], 'front-bedroom-door':[6,8],
  'family-bath-door':[10,12], 'ensuite-door':[3,5],
  'wardrobe-door':[19,21], 'master-door':[6,8],
};
for(const w of [...exteriorWalls,...partitions])for(const h of w.holes){
  const axis=w.a[0]===w.b[0]?1:0,[a,b]=openingNodes[h.id];
  const datum=axis===0?x0:z0,base=w.level==='upper'?levels.slabBottom:0;
  const sill=h.kind==='window'?3:null,lintel=6;
  h.at=datum+a*P+T-w.a[axis]; h.width=(b-a)*P-2*T;
  h.bottom=base+(sill===null?H:sill*P+T);
  h.height=base+lintel*P-T-h.bottom;
  h.moduleBounds={a,b,sill,lintel};
}
export const stair = {
...
export const rooms=[
  room('office','Gabinet / pokój gościnny','ground',rect(I.left,I.rear,A.leftX-H/2,A.officeZ-H/2),[-3.42,-1.28]),
