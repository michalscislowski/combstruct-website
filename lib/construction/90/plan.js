...
function furnishings(level){
  ...
  if(level==='ground'){
    ...
    c.push(rect(-4.50,2.40,.60,.60,'fill="#fff"'),circle(-4.20,2.70,.19),wardrobe(-4.50,1.67,.60,.65),wardrobe(-1.76,1.68,.37,1.32));
    c.push(rect(-1.035,-3.015,.60,2.39,'fill="#ede8dc"'),rect(-.435,-3.015,1.69,.60,'fill="#ede8dc"'),basin(-.28,-2.94,.64,.42));
...
function openingsMarkup(w){
  ...
  for(const h of w.holes){const start=w.a[axis]+h.at,fixed=w.a[1-axis],end=start+h.width,depth=w.depth+.03,isPatio=/patio/.test(h.kind),isDoor=/door/.test(h.kind)&&!isPatio;
    ...
    if(h.kind==='sliding-door'){
      ...
      let dir=1;if(axis===0&&(/entrance|vestibule|family-bath|rear-bedroom/.test(h.id)))dir=-1;if(axis===1&&/bath-door|utility-door|office-door/.test(h.id))dir=-1;
      const d=h.width;
