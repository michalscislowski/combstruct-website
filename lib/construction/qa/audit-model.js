...
try{
 for(const pair of pairs){
  ...
  if((hit&&hit.distance<tolerance)||vertexContact(a,b,tolerance)){a.neighbors.push(j);b.neighbors.push(i);contacts++;}
 }
...
for(const b of model.boards){
 ...
 if(['floor','slab','ceiling'].includes(b.kind))assert.equal(b.normal[1],Math.abs(b.along[project==='30'?0:2])>.9?1:-1,b.id+' short bearing slots');
