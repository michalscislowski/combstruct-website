...
export function planStock(modules, phase = 0, gaps = [], offset = 2) {
  ...
  const key = JSON.stringify([modules, phase, gaps, offset]);
  ...
  for (let at = 1; at <= modules; at++) {
    ...
    for (const state of states.values()) for (const end0 of [false, true]) for (const end1 of [false, true]) {
      ...
      for (const physical of [0, 1]) {
        ...
        const left = (layer === 1 && start === 0) || gaps.some(g => g.layer === physical && g.at === start && g.side === 'left');
        const right = (layer === 1 && at === modules) || gaps.some(g => g.layer === physical && g.at === at && g.side === 'right');
        ...
        pieces.push({pixels: count, start: start * P, layer, trim});
        starts[physical] = at;
      }
      if (!valid) continue;
