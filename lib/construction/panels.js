...
export function wallPanel(engine,w,peers,{target,base,modules,cap=true,planes=[],verticalGaps=[],continueFromBelow=()=>false,targetForRow,holes=[],frameOpenings=false}){
 ...
 for(let row=1;row<modules+(cap?1:0);row++)for(const stock of jointStock(span,phaseH,gaps,2,w.horizontalEnds))
 ...
 for(let col=0;col<=span;col++)for(const stock of jointStock(modules,phaseV,verticalGaps)){
