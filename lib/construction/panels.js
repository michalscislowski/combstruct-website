...
export function horizontalPanel(engine,target,{x0,z0,nx,nz,top,kind,walls=[],holes=[],range,planes=[]}){
 ...
 for(const axis of [0,1]){
  ...
  for(let i=1;i<maxCount;i++){
   ...
   const gaps=walls.filter(w=>w.axis===axis&&transverse>w.start+EPS&&transverse<w.end-EPS)
...
export function wallPanel(engine,w,peers,{target,base,modules,cap=true,planes=[],verticalGaps=[],continueFromBelow=()=>false,targetForRow,holes=[]}){
 ...
 for(let col=1;col<span;col++)for(const stock of jointStock(modules,1,verticalGaps)){
