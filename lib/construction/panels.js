...
export function wallPanel(engine,w,peers,{target,base,modules,capAt=modules,cap=true,verticalEnds,planes=[],verticalGaps=[],continueFromBelow=()=>false,targetForRow,holes=[],frameOpenings=false}){
 ...
 for(let col=0;col<=span;col++)for(const stock of jointStock(modules,phaseV,verticalGaps,2,verticalEnds)){
  ...
  for(const f of openStock(p,1,col*P+((p.layer^phaseV)===0?-T:0),holes))
   addBoard(target,f,Y,w.normal.clone().negate(),along,origin.clone().addScaledVector(along,col*P),{kind:w.exterior?'wall-vertical':'partition-vertical',wallId:w.id,axis:1,index:col},phaseV,planes);
 }
