...
export function horizontalPanel(engine,target,{x0,z0,nx,nz,top,kind,walls=[],holes=[],range,planes=[]}){
 ...
 for(const axis of [0,1]){
  ...
  for(let i=1;i<maxCount;i++){
   ...
   // Reserve the physical ply occupied by the wall's full upright. Swapping
   // upright plies must also swap this band in both supporting/closing panels.
   const gaps=walls.filter(w=>w.axis===axis&&((transverse>w.start+EPS&&transverse<w.end-EPS)||w.endJambs?.some(p=>Math.abs(p-transverse)<EPS)))
    .map(w=>({at:Math.round((w.node-base)/P),side:w.side,layer:w.verticalPhase??1})).filter(g=>g.at>0&&g.at<length);
   for(const stock of jointStock(length,0,gaps,2,{left:true,right:true,extraTrim:engine.lengthJoints}))for(const p of openStock(stock,axis,i*P+(stock.layer===0?-T:0),holes))
    addBoard(target,p,along,normal,thick,origin,{kind,axis,index:i},0,planes);
  }
 }
}

export function normalizeWalls(walls,{x0,x1,z0,z1,lengthJoints=false}){
 const result=walls.map(w=>{
  ...
  return{...w,axis,side,node,fixed:node+(side==='left'?1:-1)*(HALF+(lengthJoints?T:0)),originalFixed,
...
export function wallPanel(engine,w,peers,{target,base,modules,capAt=modules,cap=true,verticalEnds,planes=[],verticalGaps=[],continueFromBelow=()=>false,targetForRow,holes=[],frameOpenings=false,verticalTreatment}){
 ...
 const crosses=w.axis===0?peers.filter(o=>o.axis===1&&o.level===w.level&&(engine.lengthJoints?w.node>o.start+EPS&&w.node<o.end-EPS:w.fixed>o.start+HALF&&w.fixed<o.end-HALF)&&o.node>w.start+EPS&&o.node<w.end-EPS):[];
 for(const o of crosses)gaps.push({at:Math.round((o.node-w.start)/P),side:'seam',layer:phaseH});
 for(let row=1;row<capAt+(cap?1:0);row++)for(const stock of jointStock(span,phaseH,gaps,2,{left:true,right:true,...w.horizontalEnds,extraTrim:engine.lengthJoints}))
 ...
 const phaseV=w.verticalPhase??1;
 if(phaseV!==0&&phaseV!==1)throw Error('Invalid upright ply phase: '+w.id);
 for(let col=0;col<=span;col++)for(const stock of jointStock(modules,phaseV,verticalGaps,2,verticalEnds)){
  ...
  if(continueFromBelow(w.start+col*P)&&p.start===0&&p.layer===0){p.trim='left';if(engine.lengthJoints)p.extraStartTrim=T;}
  const treatment=verticalTreatment?.(p.layer^phaseV,col);
  for(const f of openStock(p,1,col*P+((p.layer^phaseV)===0?-T:0),holes))
   addBoard(target,f,Y,w.normal.clone().negate(),along,origin.clone().addScaledVector(along,col*P),{kind:w.exterior?'wall-vertical':'partition-vertical',wallId:w.id,axis:1,index:col,...(treatment?{roofLap:treatment.role}: {})},phaseV,treatment?.planes||planes,treatment?.regions||[]);
 }
