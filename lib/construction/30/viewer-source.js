...
window.combstructKrokusy={model,setView,setMode:setView,setDetail:setView,reset,getState:()=>({ready:true,view,detail:detail?.id||null,boardCount:boards.length,dimensions,rooms:model.rooms,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,validation:model.validation||null})};
