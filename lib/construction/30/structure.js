...
export function buildStructure(materials = {}, options = {}) {
  ...
  // Alternate long/short plies across the ridge, retaining the roof envelope.
  // Cross-rib rows keep their existing stations and half-plane boundaries.
  ...
  const ridge=lengthJoints?roofRidgeLap({pitch:35,centerY:roofCenterY,phaseOrigin:roofU,axis:0}):null;
  ...
  for(const side of [-1,1]) {
    ...
    for(let j=1;j<roofN;j++) {
      ...
      for(const stock of segments(ridge?Math.ceil((roofRun/cosine+HALF*sine/cosine+T-ridge.rafterStart)/P):slopeModules)) {
        ...
        for(const treatment of treatments)addBoard(roofPart,p,u,n,Z,origin.clone().addScaledVector(Z,j*P),{kind:'roof-slope',axis:0,index:j,roofSide:side,...(supported?{roofLap:treatment.role}:{}),...(ridgeEnd?{ridgeLap:ridgeEnd.role}:{}),detail:'Alternating ridge and roof-wall laps'},phase,[...(ridgeEnd?[planes[1],...ridgeEnd.planes]:planes),...treatment.planes],intersectRegions(treatment.regions,ridgeEnd?.regions));
      }
    }
