...
const familiesOf=data=>[...data.families,...(data.additionalFamilies||[])];
...
export function selectedPart(data,selection) {
  const family=familiesOf(data).find(f=>f.id===selection.family) || data.families[0];
...
export function mountCatalogue(root,data,onChange) {
  ...
  heading.append(node('h4',null,'Elementy zestawu'),node('span','parts-catalogue-total',quantity(data.totalBoards+(data.battens?.stockStrips||0))));
  const lead=node('p','parts-catalogue-lead','Deski i łaty w Twoim zestawie. Wybierz rodzaj i długość.');
  const families=node('div','parts-family-list');families.setAttribute('role','group');families.setAttribute('aria-label','Elementy zestawu');
  ...
  function showFamily(id,interactive=true) {
    selection.family=id;
    const family=familiesOf(data).find(f=>f.id===id);
  ...
  for(const family of familiesOf(data)) {
