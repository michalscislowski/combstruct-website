...
export function initialSelection(data) {
  ...
  return {family:family.id,variant:family.variants[0]?.id || 'fitted'};
}
export function selectedPart(data,selection) {
  ...
  return {family,variant,fitted:selection.variant==='fitted'};
}
...
// Shared controls: the same physical piece counts drive project pages and Flow.
export function mountCatalogue(root,data,onChange) {
  ...
  const lead=node('p','parts-catalogue-lead','Wybierz rodzaj, a potem długość deski.');
  ...
  const note=node('p','parts-variant-note');note.setAttribute('aria-live','polite');
  root.replaceChildren(variantHeading,variants,note,heading,lead,families);
  ...
  function select(variant,interactive=true) {
    ...
    note.textContent=variant==='fitted'?'Docinki przy otworach i krawędziach. W 3D pokazujemy przykłady.':'';
  ...
  function showFamily(id,interactive=true) {
    ...
    if(family.fitted.count){
      const b=node('button','parts-variant parts-fitted');b.type='button';b.dataset.partsVariant='fitted';
      b.append(node('span',null,'Docinki do projektu'),node('strong',null,quantity(family.fitted.count)));
      b.addEventListener('click',()=>select('fitted'));buttons.push(b);
    }
    variants.replaceChildren(...buttons);
    select(family.variants[0]?.id || 'fitted',interactive);
  }
