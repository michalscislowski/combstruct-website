import {stockBounds,endTreatment} from '../construction/stock-profile.js';
...
export function sourceStockKey(board) {
  return JSON.stringify([board.surface,board.wallId||'',board.kind,board.axis,board.index,
    board.layer,board.stockLayer,board.stock.pixels,board.trim,endTreatment(board.stock),
...
export function deliveryStock(boards) {
  ...
  for(const board of boards){
    ...
    if(!sources.has(key))sources.set(key,{
      id:`stock-${sources.size+1}`,key,stock:{pixels:board.stock.pixels,...endTreatment(board.stock)},
  ...
  return [...sources.values()].map(stock=>({...stock,family:boardFamily(stock),variantKey:nativeVariantKey(stock),
    lengthM:(()=>{const [lo,hi]=stockBounds({...stock.stock,trim:stock.trim},false);return hi-lo})()}));
}
