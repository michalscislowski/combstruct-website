import {endTreatment} from '../construction/stock-profile.js';
...
export function nativeVariantKey(board) {
  const keys = [false, true].map(flip => {
    ...
    const treatment=endTreatment(board.stock),ends=[treatment.extendStart||0,treatment.extendEnd||0,treatment.extraStartTrim||0,treatment.extraEndTrim||0];
    if(flip){[ends[0],ends[1]]=[ends[1],ends[0]];[ends[2],ends[3]]=[ends[3],ends[2]];}
    return JSON.stringify([board.stock.pixels, board.trim ? end(board.trim) : null, cuts,...(ends.some(Boolean)?[ends]:[])]);
  });
  return keys.sort()[0];
}
