...
import partsData from '../../site/assets/parts/30.json';
import {partScene} from '../parts/scene.js';
import {initialSelection} from '../parts/catalogue.js';

export const productionParts=partsData;
...
export function manufacturingScene(stage,selection=initialSelection(partsData)){
 if(stage===2){
  const view=partScene(partsData,selection);
  return {...view,getManufacturingState:()=>({stage,...view.getPartsState(),example:partsData.name})};
 }
