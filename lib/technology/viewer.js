...
let roofOptions={pitch:35,eave:true,knee:true};
...
function buildScene(){
 ...
 try{
  ...
  current=technology==='combstruct'?manualScene(topic,variant,roofOptions):technology==='bim'?bimScene(bimView,selectInfo):technology==='robotics'?roboticsScene(roboticsProcess):manufacturingScene(flowStage,productionSelection);
  ...
  if(technology==='combstruct'&&topic.scene==='roof-wall'){current.spread?.(Number($('#spread').value));current.insert?.(Number($('#roof-joint-insert').value));}
...
function updateTopic(){
 ...
 $('#roof-joint-controls').hidden=topic.scene!=='roof-wall';$('#roof-joint-insert').value=0;$('#roof-joint-insert-value').textContent='0 mm';
...
window.combstructTechnology={getState:()=>({ready:!!renderer,technology,topic:topic.id,variant:variant.id,bimView,flowStage,roofJoint:technology==='combstruct'&&topic.scene==='roof-wall'?{...roofOptions,slabBeveled:false,boards:current?.roofJoint?.boards.length}:null,robotics:technology==='robotics'?{...current?.getRoboticsState?.(),playing:roboticsPlaying,camera:{following:followsFactory(),position:camera?.position.toArray(),target:controls?.target.toArray()}}:null,manufacturing:current?.getManufacturingState?.(),selected:current?.getSelected?.(),wall:current?.getWallState?.(),meshCount:current?.root.children.length,drawCalls:renderer?.info.render.calls}),topics:topics.map(t=>t.id),showTopic,showTechnology,seekRobotics,reset:fit};
