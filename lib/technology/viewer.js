...
function selectInfo(info){$('#element-id').textContent=info.id;$('#element-type').textContent=info.type;$('#element-surface').textContent=info.surface;$('#element-length').textContent=info.length;}
function buildScene(){
 if(!renderer)return;
 try{
  ...
  if(technology==='bim')current.finish?.(Number($('#finish').value));
...
function startRenderer(){
 try{
  ...
  renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,preserveDrawingBuffer:true,powerPreference:'low-power'});renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.6));renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.07;
  viewport.prepend(renderer.domElement);renderer.domElement.tabIndex=0;renderer.domElement.setAttribute('aria-label','Model 3D. Przeciągnij, aby obrócić; przewiń, aby przybliżyć. Klawisz Home przywraca widok. W sekcji BIM kliknij deskę, aby ją wybrać.');
...
function showTechnology(next,{hash=true}={}){
 ...
 if(hash)history.pushState(null,'',`#${next==='combstruct'?topic.id:next}`);
...
function fromLocation(){const id=location.hash.slice(1);if(id==='bim'||id==='mycelium')showTechnology(id,{hash:false});else showTopic(id,{hash:false});}
...
$$('[data-bim-view]').forEach(b=>b.addEventListener('click',()=>{bimView=b.dataset.bimView;$$('[data-bim-view]').forEach(t=>t.setAttribute('aria-pressed',String(t===b)));$('#finish-control').hidden=bimView!=='wall';buildScene();}));
$('#finish').addEventListener('input',e=>{current?.finish?.(Number(e.target.value));invalidate();});$('#next-element').addEventListener('click',()=>{current?.next?.();invalidate();});
...
window.combstructTechnology={getState:()=>({ready:!!renderer,technology,topic:topic.id,variant:variant.id,bimView,materialStage,selected:current?.getSelected?.(),meshCount:current?.root.children.length,drawCalls:renderer?.info.render.calls}),topics:topics.map(t=>t.id),showTopic,showTechnology,reset:fit};
