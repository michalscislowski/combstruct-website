...
import {manufacturingScene,flowSteps,productionFamilies,productionMaterial} from './manufacturing.js';
...
let productionFamily=productionFamilies[0].id;
...
function buildScene(){
 ...
 try{
  ...
  current=technology==='combstruct'?manualScene(topic,variant):technology==='bim'?bimScene(bimView,selectInfo):manufacturingScene(flowStage,productionFamily);
...
function showTechnology(next,{hash=true}={}){
 technology=next;$('.technology-cta').hidden=next==='flow';
...
$('#production-total').textContent=`${productionFamilies.reduce((n,f)=>n+f.count,0).toLocaleString('pl-PL')} szt.`;
$('#production-material').textContent=`Materiał w przeliczeniu na deski 2,5 m: ${productionMaterial.fullBoards.toLocaleString('pl-PL')} szt.`;
$('#production-families').replaceChildren(...productionFamilies.map(f=>{
 const button=document.createElement('button');button.type='button';button.dataset.productionFamily=f.id;button.setAttribute('aria-pressed',String(f.id===productionFamily));
 const name=document.createElement('span');name.textContent=f.name;
 const count=document.createElement('strong');count.textContent=`${f.count.toLocaleString('pl-PL')} szt.`;button.append(name,count);
 button.addEventListener('click',()=>{
  productionFamily=f.id;$$('[data-production-family]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.productionFamily===f.id)));buildScene();
  const mount=$('#flow-mount'),rect=mount.getBoundingClientRect();
  if(rect.top<80||rect.bottom>innerHeight)mount.scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});
 });return button;
}));
...
window.combstructTechnology={getState:()=>({ready:!!renderer,technology,topic:topic.id,variant:variant.id,bimView,flowStage,manufacturing:current?.getManufacturingState?.(),selected:current?.getSelected?.(),wall:current?.getWallState?.(),meshCount:current?.root.children.length,drawCalls:renderer?.info.render.calls}),topics:topics.map(t=>t.id),showTopic,showTechnology,reset:fit};
