...
window.combstructTechnology={getState:()=>({ready:!!renderer,technology,topic:topic.id,variant:variant.id,bimView,flowStage,robotics:technology==='robotics'?{...current?.getRoboticsState?.(),playing:roboticsPlaying,camera:{following:followsFactory(),position:camera?.position.toArray(),target:controls?.target.toArray()}}:null,manufacturing:current?.getManufacturingState?.(),selected:current?.getSelected?.(),wall:current?.getWallState?.(),meshCount:current?.root.children.length,drawCalls:renderer?.info.render.calls}),topics:topics.map(t=>t.id),showTopic,showTechnology,seekRobotics,reset:fit};
