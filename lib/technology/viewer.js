...
function fit(){
 ...
 const damping=controls.enableDamping;controls.enableDamping=false;controls.update();controls.target.copy(center);camera.position.copy(center).addScaledVector(direction,Math.max(.8,distance*1.14*(current.distanceScale||1)));controls.update();
...
function buildScene(){
 ...
 try{
  ...
  current=technology==='combstruct'?manualScene(topic,variant):technology==='bim'?bimScene(bimView,selectInfo):technology==='robotics'?roboticsScene(roboticsProcess):manufacturingScene(flowStage,productionSelection);
...
function updateTopic(){
 ...
 $('#spread').value=0;$('#crossing').checked=!!topic.crossing;$('#previous-topic').disabled=index===0;$('#next-topic').disabled=index===topics.length-1;
...
function updateRobotics(){
 ...
 if(i!==roboticsStep){roboticsStep=i;$('#robotics-title').textContent=def.steps[i].title;$('#robotics-text').textContent=def.steps[i].text;$$('[data-robotics-step]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.roboticsStep)===i)));if(roboticsDetail&&current?.getDetailFocus){current.focus=current.getDetailFocus();current.direction=current.detailDirection||current.direction;fit();}}
...
function seekRobotics(p){roboticsProgress=Math.min(1,Math.max(0,p));roboticsPlaying=false;roboticsStamp=0;updateRobotics();invalidate();}
...
$('#robotics-play').addEventListener('click',()=>{if(roboticsProgress===1)roboticsProgress=0;roboticsPlaying=!roboticsPlaying;roboticsStamp=0;updateRobotics();invalidate();});
$('#robotics-replay').addEventListener('click',()=>{roboticsProgress=0;roboticsPlaying=!matchMedia('(prefers-reduced-motion: reduce)').matches;roboticsStamp=0;updateRobotics();invalidate();});
...
window.combstructTechnology={getState:()=>({ready:!!renderer,technology,topic:topic.id,variant:variant.id,bimView,flowStage,robotics:technology==='robotics'?{...current?.getRoboticsState?.(),playing:roboticsPlaying}:null,manufacturing:current?.getManufacturingState?.(),selected:current?.getSelected?.(),wall:current?.getWallState?.(),meshCount:current?.root.children.length,drawCalls:renderer?.info.render.calls}),topics:topics.map(t=>t.id),showTopic,showTechnology,seekRobotics,reset:fit};
