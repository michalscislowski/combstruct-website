...
import {selectRobotOperations,createRobotInventory,remainingRobotStock} from './robotics-assembly.js';
...
export const roboticsProcesses={
 ...
 site:{title:'Robotyczny montaż',duration:60,steps:[
  {at:0,label:'Podłoga',title:'Wybierz. Pobierz. Osadź.',text:'Model wskazuje potrzebny profil i długość. Robot sam wybiera właściwy stos, pobiera z niego deskę i montuje ją w konstrukcji. Bez ręcznego podawania części w kolejności.'},
...
function siteScene(){
 ...
 const operations=selectRobotOperations(records,starts),inventory=createRobotInventory(records,operations);
 ...
 // Persistent stacks of native boards on low dunnage, grouped by profile and
 // length. No feed table and no shape-changing single-board source.
 const stackBase=.16,stockViews=new Map();
 for(const stack of inventory.stacks){
  stack.position=V(stack.column===0?-1.7:1.7,stackBase,d.halfDepth+1.25+stack.row*.53);
  const length=stack.prepared.dimensions[0],group=new THREE.Group();group.position.copy(stack.position);root.add(group);
  for(const x of [-length*.32,length*.32])meshBox(group,.10,.12,.34,x,-.06,0,wood);
  const boards=[];
  for(let i=0;i<stack.initial;i++){
   const board=outlined(stack.prepared.geometry,wood);board.position.y=(i+.5)*T;group.add(board);boards.push(board);
  }
  const selection=new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(length+.16,.008,H+.13)),new THREE.LineBasicMaterial({color:'#2a9b82'}));
  selection.position.y=-.125;group.add(selection);
  stockViews.set(stack.id,{boards,selection,label:null,count:-1});
 }
 sign(root,'Deski zwykłe',-1.7,.85,d.halfDepth+.65,2.6,'#254331');
 sign(root,'Deski zakończeniowe',1.7,.85,d.halfDepth+.65,2.6,'#254331');
 const moving=outlined(new THREE.BoxGeometry(.01,.01,.01),mat('#379b88'));root.add(moving);
 moving.geometry.dispose();moving.children[0].geometry.dispose();
 const preparedEdges=new Map(inventory.picks.map(pick=>[pick.index,new THREE.EdgesGeometry(pick.prepared.geometry,30)]));
 const safeY=d.roofRidgeY+1.25,park=V(0,safeY,zmax-.6),contact=V(0,.205,-H/4);
 let state={};
 function stockPosition(pick,counts){return pick.stack.position.clone().add(V(0,(counts.get(pick.stack.id)-.5)*T,0));}
 function showInventory(counts,selected,picking){
  for(const stack of inventory.stacks){
   const view=stockViews.get(stack.id),count=counts.get(stack.id),active=stack.id===selected;
   view.selection.visible=active;
   view.boards.forEach((board,i)=>{board.visible=i<count;board.material=active&&picking&&i===count-1?moving.material:wood;});
   if(view.count!==count){
    if(view.label){root.remove(view.label);view.label.material.map.dispose();view.label.material.dispose();}
    view.label=sign(root,`${stack.id} · ${stack.prepared.dimensions[0].toFixed(2)} m · ${count}×`,stack.position.x,stackBase+count*T+.07,stack.position.z+.20,1.65,'#254331');
    view.count=count;
   }
  }
 }
 function positionTool(pose,rotation){
  const tool=pose.clone().add(contact.clone().applyQuaternion(rotation));bridge.position.z=tool.z;trolley.position.x=tool.x;
  ...
  clampFrame.position.copy(tool);clampFrame.quaternion.copy(rotation);gripJoint.position.copy(tool);return tool;
 }
 function inventoryState(counts){return inventory.stacks.map(stack=>({id:stack.id,profile:stack.profile,length:stack.prepared.dimensions[0],initial:stack.initial,remaining:counts.get(stack.id),position:stack.position.toArray()}));}
 function update(value){
  const p=clamp(value),step=Math.min(4,Math.floor(p*5)),local=clamp(p*5-step);
  if(step===4){
   house.assembly(1);moving.visible=false;
   const counts=remainingRobotStock(inventory,inventory.picks.length);showInventory(counts,null,false);
   const tool=positionTool(park,new THREE.Quaternion());
   state={process:'site',progress:p,step,installed:records.length,total:records.length,complete:true,inventory:inventoryState(counts),picked:inventory.picks.length,toolPosition:tool.toArray()};return;
  }
  // Selected operations from the real house sequence. Stock consumption belongs
  // to these visible picks; skipped construction prefixes never consume it.
  const cycles=4,k=Math.min(cycles-1,Math.floor(local*cycles)),f=clamp(local*cycles-k),ordinal=step*cycles+k;
  const pick=inventory.picks[ordinal],index=pick.index,{target,rotation:installRotation,geometry,dimensions:boardDimensions}=pick.prepared;
  const lifted=f>=.16,placed=f>=.86,before=remainingRobotStock(inventory,ordinal),counts=remainingRobotStock(inventory,ordinal+(lifted?1:0));
  const pickup=stockPosition(pick,before),next=inventory.picks[ordinal+1];
  const nextPickup=next?stockPosition(next,remainingRobotStock(inventory,ordinal+1)):park.clone().add(V(0,-.5,0));
  showInventory(counts,pick.stack.id,!lifted);house.assembly((index+(placed?1:0))/records.length);
  moving.geometry=geometry;moving.children[0].geometry=preparedEdges.get(index);
  const abovePick=pickup.clone().setY(safeY),aboveTarget=target.clone().setY(safeY),aboveNext=nextPickup.clone().setY(safeY),pose=V();
  ...
  else if(f<.98)pose.lerpVectors(aboveTarget,aboveNext,span(f,.92,.98));
  else pose.lerpVectors(aboveNext,nextPickup.clone().add(V(0,.5,0)),span(f,.98,1));
  moving.visible=lifted&&!placed;moving.position.copy(pose);moving.quaternion.identity().slerp(installRotation,span(f,.35,.60)*(1-span(f,.92,.98)));
  const tool=positionTool(pose,moving.quaternion);
  state={process:'site',progress:p,step,installed:index+(placed?1:0),total:records.length,activeBoard:records[index].id,boardKind:records[index].kind,boardProfile:pick.stack.profile,boardDimensions,nativeStock:true,pickupGap:pickup.y+geometry.boundingBox.min.y-(stackBase+(before.get(pick.stack.id)-1)*T),pickupPosition:pickup.toArray(),support:sequence.assembly[index][1],carrying:moving.visible,sourceVisible:!lifted,phase:f<.16?'pick':f<.6?'carry':f<.86?'place':'release',complete:false,selectedOperations:true,selectedStack:pick.stack.id,nextStack:next?.stack.id||null,inventory:inventoryState(counts),picked:ordinal+(lifted?1:0),toolPosition:tool.toArray()};
 }
 update(0);
 return{root,direction:V(1.5,1.15,2),distanceScale:.83,focus:new THREE.Box3(V(xmin-1,-.2,zmin-.4),V(xmax+.7,top+.35,zmax+.4)),annotation:'Wybrane operacje · Combstruct 30',update,getRoboticsState:()=>state,dispose(){
  for(const pick of inventory.picks)pick.prepared.geometry.dispose();
  for(const edge of preparedEdges.values())edge.dispose();
 }};
}
