...
export function manualScene(topic,variant){
 ...
 // The short upright ends at the underside of the ceiling. Its 24 cm
 // reservation is occupied by the real ceiling rib, which must stay visible
 // in this detail along with the upright and roof pair.
 if(topic.scene==='roof-wall')return fromRecords([{surface:'right',axis:1,index:3},{surface:'roof-right',axis:0,index:4},{surface:'Sufit',axis:0,index:3,material:brown}],V(d.halfWidth-HALF,d.ceilingTop,-d.halfDepth+3*P),{plane:2,direction:V(1.5,1.2,2),size:1.2});
